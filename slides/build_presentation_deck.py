...
def add_notes(slide, explanation, time_mins=15, teaching_tips="", questions="", goal=None, demo_lab=None, typical_errors=None):
    """Aggiunge note relatore complete e dettagliate con tutte le 6 sezioni richieste."""
    ...
    if goal:
        ...
        # Estrai la prima frase significativa come obiettivo didattico
        frasi = [f.strip() for f in explanation.split('.') if f.strip()]
        g = (frasi[0] + '.') if frasi else explanation
        
    # Sezione Demo / Laboratorio
    dl = demo_lab if demo_lab else (
        "Esecuzione live della demo a schermo e affiancamento degli studenti nello svolgimento dell'esercitazione associata nel notebook/script di laboratorio."
    )
    
    # Sezione Errori Tipici
    te = typical_errors if typical_errors else (
        "Confusione sintattica tra formati, mancata gestione delle eccezioni o dimenticanza nell'uso di .copy() sulle viste dei dati."
    )
    
    tf.text = (
        f"🎯 OBIETTIVO DIDATTICO:\n{g}\n\n"
        f"⏱️ TIMING:\n{time_mins} Minuti\n\n"
        f"📘 GUIDA DOCENTE:\n{explanation}\n\n"
        f"💻 DEMO / LABORATORIO:\n{teaching_tips}\n{dl}\n\n"
        f"⚠️ ERRORI TIPICI:\n{te}\n\n"
